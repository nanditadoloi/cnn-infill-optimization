...
interim_perm_file = open('interim_perm_.txt','r')
...
new_data = (new_data_permx + new_data_permy + new_data_permz)/3.0
new_data = np.log(new_data)
plt.imshow(new_data[:,:,1],cmap='jet',vmin=np.min(new_data[:,:,1])*1.)
plt.colorbar()
plt.show()
